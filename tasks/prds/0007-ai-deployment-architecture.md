# PRD 0007: AI 배포 아키텍처 및 Fallback 전략

**Version**: 1.0
**Date**: 2025-12-08
**Author**: Claude Code
**Status**: Draft
**Related Issues**: #103, #101, #96

---

## 1. 문제 정의

### 1.1 현재 문제점

| 문제 | 원인 | 영향 |
|------|------|------|
| Vercel에서 Local AI 연결 불가 | HTTPS → HTTP Mixed Content 차단 | 배포 환경에서 Local AI 사용 불가 |
| WebLLM 무한 로딩 | 2.4GB 모델 다운로드 + 초기화 시간 | 사용자 이탈, UX 저하 |
| Fallback Content 미작동 | WebLLM 실패 시 처리 로직 부재 | AI 실패 시 아무 것도 표시 안됨 |

### 1.2 환경별 상황

```
┌─────────────────────────────────────────────────────────────────┐
│                     현재 아키텍처 문제                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  [Vercel HTTPS]                    [Local AI HTTP]              │
│       │                                  │                      │
│       └──────── ❌ Mixed Content ────────┘                      │
│                    (브라우저 차단)                                │
│                                                                 │
│  [WebLLM]                                                       │
│       │                                                         │
│       └──────── ❌ 무한 로딩 (타임아웃 없음) ─────────────────────│
│                                                                 │
│  [Fallback Content]                                             │
│       │                                                         │
│       └──────── ❌ 호출 안됨 (WebLLM이 끝나지 않음) ──────────────│
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 2. 솔루션 개요

### 2.1 배포 환경별 전략

| 환경 | AI 엔진 | 구현 방식 |
|------|---------|----------|
| **개발 (localhost)** | Local AI → WebLLM → Fallback | 현재 구조 유지 |
| **프로덕션 (Vercel)** | WebLLM → Cloud API → Fallback | Local AI 비활성화 |
| **사내 배포 (Docker)** | Local AI → WebLLM → Fallback | 동일 origin 배포 |

### 2.2 AI 엔진 우선순위 (최종)

```
┌─────────────────────────────────────────────────────────────────┐
│                      AI 엔진 Fallback 체인                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. Local AI (vLLM)                                             │
│     ├─ 조건: HTTP 환경 또는 HTTPS+HTTPS                          │
│     ├─ 타임아웃: 5초                                             │
│     └─ 실패 시 → 2번으로                                         │
│                                                                 │
│  2. WebLLM (브라우저)                                            │
│     ├─ 조건: WebGPU 지원                                         │
│     ├─ 로딩 타임아웃: 30초 (첫 로딩), 10초 (재시도)                │
│     ├─ 생성 타임아웃: 60초                                       │
│     └─ 실패/타임아웃 시 → 3번으로                                 │
│                                                                 │
│  3. Cloud API (Gemini)                                          │
│     ├─ 조건: API 키 설정됨                                       │
│     ├─ 타임아웃: 30초                                            │
│     └─ 실패 시 → 4번으로                                         │
│                                                                 │
│  4. Fallback Content                                            │
│     ├─ 조건: 항상 사용 가능                                       │
│     └─ 원본 텍스트 + 자동 생성 퀴즈                               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 3. 솔루션 A: 배포 환경 Local AI 지원

### 3.1 Option A-1: Cloudflare Tunnel (권장)

```
┌─────────────────────────────────────────────────────────────────┐
│                  Cloudflare Tunnel 구조                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  [Vercel HTTPS]                                                 │
│       │                                                         │
│       ▼                                                         │
│  [https://localai.your-domain.com]  ← Cloudflare Tunnel         │
│       │                                                         │
│       ▼                                                         │
│  [cloudflared]  ←  사내 서버에서 실행                             │
│       │                                                         │
│       ▼                                                         │
│  [http://localhost:8001]  ← vLLM 서버                            │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

**장점**:
- 무료 (Cloudflare Zero Trust)
- SSL 자동 적용
- 외부 포트 오픈 불필요

**설정 방법**:
```bash
# 사내 서버에서 실행
cloudflared tunnel create localai
cloudflared tunnel route dns localai localai.your-domain.com
cloudflared tunnel run --url http://localhost:8001 localai
```

### 3.2 Option A-2: Nginx Reverse Proxy + Let's Encrypt

```nginx
# /etc/nginx/sites-available/localai
server {
    listen 443 ssl;
    server_name localai.your-domain.com;

    ssl_certificate /etc/letsencrypt/live/localai.your-domain.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/localai.your-domain.com/privkey.pem;

    location / {
        proxy_pass http://127.0.0.1:8001;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }
}
```

---

## 4. 솔루션 B: Docker 사내 배포 (구현 완료)

### 4.1 아키텍처

```
┌─────────────────────────────────────────────────────────────────┐
│                   사내 Docker 배포 구조                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────────────┐       ┌─────────────────────────┐      │
│  │   OJT Master 앱     │       │   AI 서버 (별도 프로젝트) │      │
│  │   (Docker)          │ ───▶  │   vLLM Qwen3-4B         │      │
│  │   Port: 80          │       │   10.10.100.209:8001    │      │
│  └─────────────────────┘       └─────────────────────────┘      │
│           │                                                     │
│           │ Supabase (외부 클라우드)                              │
│           ▼                                                     │
│  ┌─────────────────────┐                                       │
│  │   사내 사용자        │                                       │
│  │   http://<서버IP>/   │                                       │
│  └─────────────────────┘                                       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 4.2 docker-compose.yml (간소화)

```yaml
# local-ai-server/docker-compose.yml
services:
  app:
    build:
      context: ..
      dockerfile: local-ai-server/Dockerfile.dev
      args:
        - VITE_SUPABASE_URL=${VITE_SUPABASE_URL}
        - VITE_SUPABASE_PUBLISHABLE_KEY=${VITE_SUPABASE_PUBLISHABLE_KEY:-${VITE_SUPABASE_ANON_KEY}}
        - VITE_LOCAL_AI_URL=${VITE_LOCAL_AI_URL}
        - VITE_R2_WORKER_URL=${VITE_R2_WORKER_URL:-}
    container_name: ojt-master
    ports:
      - "${APP_PORT:-80}:3000"
    restart: unless-stopped
```

### 4.3 Dockerfile.dev (Node.js serve 기반)

```dockerfile
# Build Stage
FROM node:20-alpine AS builder
WORKDIR /app
RUN corepack enable && corepack prepare pnpm@9 --activate
COPY package.json pnpm-lock.yaml pnpm-workspace.yaml ./
COPY src-vite/package.json ./src-vite/
RUN pnpm install --frozen-lockfile
COPY src-vite/ ./src-vite/
ARG VITE_SUPABASE_URL
ARG VITE_LOCAL_AI_URL
# ... 빌드 ...
RUN pnpm --filter src-vite build

# Production Stage
FROM node:20-alpine
RUN npm install -g serve
COPY --from=builder /app/src-vite/dist ./dist
EXPOSE 3000
CMD ["serve", "-s", "dist", "-l", "3000"]
```

### 4.4 배포 명령어

```bash
cd local-ai-server
cp .env.example .env
# .env 편집: Supabase 정보 입력

docker compose up -d --build
# 접속: http://<서버IP>/
```

---

## 5. 솔루션 C: WebLLM + Fallback 개선

### 5.0 핵심 UX: 사용자 주도 Fallback 전환

**요구사항**: 사용자가 WebLLM 대기 중 언제든지 Fallback으로 즉시 전환 가능

```
┌─────────────────────────────────────────────────────────────────┐
│                    WebLLM 로딩 중 UI                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  🌐 WebLLM 모델 로딩 중...                                       │
│                                                                 │
│  ████████████░░░░░░░░░░░ 48%                                    │
│  다운로드: 1.2GB / 2.4GB                                         │
│                                                                 │
│  ⏱️ 예상 대기 시간: 약 2분                                       │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │                                                         │    │
│  │  [🔄 계속 대기]        [⏭️ Fallback으로 건너뛰기]        │    │
│  │                                                         │    │
│  │  💡 Fallback: AI 분석 없이 원본 텍스트로 즉시 등록       │    │
│  │                                                         │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

**구현 방식**: AbortController 기반 취소

```javascript
// WebLLM 취소 가능한 로딩
const abortController = new AbortController();

// 사용자가 "Fallback으로 건너뛰기" 클릭
const handleSkipToFallback = () => {
  abortController.abort();  // WebLLM 즉시 취소
  setStatus('fallback');
  generateFallbackContent();
};

// WebLLM with AbortSignal
try {
  await initWebLLM(onProgress, { signal: abortController.signal });
} catch (error) {
  if (error.name === 'AbortError') {
    console.log('[WebLLM] 사용자에 의해 취소됨');
    return null;  // Fallback으로 전환
  }
  throw error;
}
```

### 5.1 현재 문제

```javascript
// 현재 코드 (문제)
async function generateWithWebLLMEngine(contentText, title, onProgress) {
  const { initWebLLM } = await import('./webllm.js');
  await initWebLLM(onProgress);  // ❌ 타임아웃 없음! 무한 대기 가능
  // ...
}
```

### 5.2 개선된 Fallback 로직

```javascript
// 개선된 코드
const WEBLLM_LOAD_TIMEOUT = 30000;  // 30초
const WEBLLM_GENERATE_TIMEOUT = 60000;  // 60초

async function generateWithWebLLMEngine(contentText, title, onProgress) {
  const { initWebLLM, generateWithWebLLM } = await import('./webllm.js');

  // 1. WebLLM 로딩 with 타임아웃
  try {
    await Promise.race([
      initWebLLM(onProgress),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error('WebLLM 로딩 타임아웃 (30초)')), WEBLLM_LOAD_TIMEOUT)
      )
    ]);
  } catch (error) {
    console.warn('[WebLLM] 로딩 실패/타임아웃:', error.message);
    throw error;  // Fallback으로 전환
  }

  // 2. 콘텐츠 생성 with 타임아웃
  try {
    const result = await Promise.race([
      generateWithWebLLM(contentText, title, onProgress),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error('WebLLM 생성 타임아웃 (60초)')), WEBLLM_GENERATE_TIMEOUT)
      )
    ]);
    return result;
  } catch (error) {
    console.warn('[WebLLM] 생성 실패/타임아웃:', error.message);
    throw error;
  }
}
```

### 5.3 generateOJTContent 개선

```javascript
export async function generateOJTContent(contentText, title, _step, _total, onProgress) {
  const startTime = Date.now();
  const errors = [];

  // 1. Local AI 시도 (HTTP 환경에서만)
  if (isLocalAIAvailable()) {
    try {
      onProgress?.('Local AI 서버 연결 확인 중...');
      const result = await withTimeout(
        generateWithLocalAI(contentText, title, onProgress),
        5000,
        'Local AI 연결 타임아웃'
      );
      if (result) {
        onProgress?.('✅ Local AI 생성 완료');
        return result;
      }
    } catch (error) {
      errors.push({ engine: 'localai', error: error.message });
      onProgress?.(`Local AI 실패: ${error.message}`);
    }
  }

  // 2. WebLLM 시도
  try {
    onProgress?.('WebLLM 엔진 준비 중...');
    const result = await withTimeout(
      generateWithWebLLMEngine(contentText, title, onProgress),
      90000,  // 로딩 30초 + 생성 60초
      'WebLLM 타임아웃'
    );
    if (result) {
      onProgress?.('✅ WebLLM 생성 완료');
      return result;
    }
  } catch (error) {
    errors.push({ engine: 'webllm', error: error.message });
    onProgress?.(`WebLLM 실패: ${error.message}`);
  }

  // 3. Cloud API 시도 (설정된 경우)
  if (isCloudAPIConfigured()) {
    try {
      onProgress?.('Cloud API로 생성 중...');
      const result = await withTimeout(
        generateWithCloudAPI(contentText, title, onProgress),
        30000,
        'Cloud API 타임아웃'
      );
      if (result) {
        onProgress?.('✅ Cloud API 생성 완료');
        return result;
      }
    } catch (error) {
      errors.push({ engine: 'cloudapi', error: error.message });
      onProgress?.(`Cloud API 실패: ${error.message}`);
    }
  }

  // 4. Fallback Content (최후의 수단)
  onProgress?.('⚠️ AI 분석 실패 - Fallback 콘텐츠 생성 중...');
  const fallback = createFallbackContent(contentText, title, errors);
  fallback.generation_time = Date.now() - startTime;
  fallback.fallback_reason = errors.map(e => `${e.engine}: ${e.error}`).join(', ');

  return fallback;
}
```

---

## 6. UI 목업 디자인

### 6.1 상태별 UI 플로우

```
┌─────────────────────────────────────────────────────────────────┐
│                    AI 엔진 상태 UI 플로우                         │
├─────────────────────────────────────────────────────────────────┤

[초기 상태]
┌─────────────────────────────────────┐
│ 🔄 AI 엔진 초기화 중...              │
│                                     │
│ ▪ Local AI 연결 확인 중...           │
│   [=========>          ] 45%        │
│                                     │
│ 예상 소요 시간: 약 5초               │
└─────────────────────────────────────┘
          │
          ▼ (Local AI 성공)
┌─────────────────────────────────────┐
│ 🖥️ Local AI 연결됨        [새로고침] │
│                                     │
│ ✅ 모델: Qwen/Qwen3-4B               │
│    서버: http://10.10.100.209:8001   │
│                                     │
│ ┌───────────────────────────────┐   │
│ │ Local AI 장점:                │   │
│ │ • 사내 서버에서 직접 처리      │   │
│ │ • 빠른 응답 속도               │   │
│ │ • 대규모 모델 사용 가능        │   │
│ └───────────────────────────────┘   │
└─────────────────────────────────────┘

          ▼ (Local AI 실패 - HTTPS 환경)
┌─────────────────────────────────────┐
│ ⚠️ Local AI 사용 불가     [새로고침] │
│                                     │
│ 원인: HTTPS 환경에서 HTTP 서버 차단   │
│                                     │
│ ┌───────────────────────────────┐   │
│ │ 💡 해결 방법:                  │   │
│ │ 1. 개발 서버(localhost) 사용   │   │
│ │ 2. Docker 사내 배포            │   │
│ │ 3. Cloudflare Tunnel 설정     │   │
│ └───────────────────────────────┘   │
│                                     │
│ [WebLLM으로 대체]  [건너뛰기]        │
└─────────────────────────────────────┘

          ▼ (WebLLM 시작)
┌─────────────────────────────────────┐
│ 🌐 WebLLM 모델 로딩 중...            │
│                                     │
│ ┌───────────────────────────────┐   │
│ │ ████████████░░░░░░░░ 58%      │   │
│ │ 다운로드: 1.4GB / 2.4GB        │   │
│ │ 예상 시간: 약 2분               │   │
│ └───────────────────────────────┘   │
│                                     │
│ ⏱️ 타임아웃까지: 25초                │
│                                     │
│ [취소하고 Fallback 사용]             │
└─────────────────────────────────────┘

          ▼ (WebLLM 타임아웃/실패)
┌─────────────────────────────────────┐
│ ❌ WebLLM 로딩 실패       [다시시도] │
│                                     │
│ 사유: 30초 타임아웃 초과             │
│                                     │
│ ┌───────────────────────────────┐   │
│ │ 📋 Fallback 옵션:              │   │
│ │                               │   │
│ │ ○ Cloud API 사용 (Gemini)     │   │
│ │   → API 키 필요, 빠른 응답     │   │
│ │                               │   │
│ │ ● 원본 텍스트로 등록           │   │
│ │   → AI 분석 없이 원문 그대로   │   │
│ │   → 퀴즈 자동 생성 (기본형)    │   │
│ └───────────────────────────────┘   │
│                                     │
│ [Fallback으로 진행]                  │
└─────────────────────────────────────┘

          ▼ (Fallback Content)
┌─────────────────────────────────────┐
│ 📄 Fallback 모드           [재시도] │
│                                     │
│ AI 분석 없이 원본 텍스트를 사용합니다 │
│                                     │
│ ┌───────────────────────────────┐   │
│ │ 포함 항목:                     │   │
│ │ ✅ 원본 텍스트 (섹션 1개)       │   │
│ │ ✅ 기본 퀴즈 10개 (자동 생성)   │   │
│ │ ⚠️ AI 요약 없음                │   │
│ └───────────────────────────────┘   │
│                                     │
│ 💡 나중에 AI 연결 후 재생성 가능     │
└─────────────────────────────────────┘
```

### 6.2 콘텐츠 생성 중 진행 표시

```
┌─────────────────────────────────────────────────────────────────┐
│                   콘텐츠 생성 진행 표시                          │
├─────────────────────────────────────────────────────────────────┤

[정상 플로우]
┌─────────────────────────────────────┐
│ 📝 OJT 콘텐츠 생성 중...             │
│                                     │
│ ┌───────────────────────────────┐   │
│ │ ✅ 1. Local AI 연결 확인       │   │
│ │ ✅ 2. 텍스트 분석 완료         │   │
│ │ 🔄 3. 섹션 구조화 중... (45초)  │   │
│ │ ○ 4. 퀴즈 생성                │   │
│ │ ○ 5. 검증 및 저장             │   │
│ └───────────────────────────────┘   │
│                                     │
│ 사용 엔진: Local AI (Qwen3-4B)       │
│ 경과 시간: 32초                      │
└─────────────────────────────────────┘

[Fallback 플로우]
┌─────────────────────────────────────┐
│ 📝 OJT 콘텐츠 생성 중...             │
│                                     │
│ ┌───────────────────────────────┐   │
│ │ ❌ 1. Local AI: 연결 실패      │   │
│ │ ❌ 2. WebLLM: 타임아웃 (30초)   │   │
│ │ ⏭️ 3. Cloud API: 미설정        │   │
│ │ 🔄 4. Fallback 콘텐츠 생성 중   │   │
│ └───────────────────────────────┘   │
│                                     │
│ ⚠️ AI 분석 실패                     │
│ 원본 텍스트로 등록됩니다             │
└─────────────────────────────────────┘
```

### 6.3 에러 상태 상세

```
┌─────────────────────────────────────┐
│ ❌ AI 엔진 오류 상세        [닫기]  │
├─────────────────────────────────────┤
│                                     │
│ 시도한 엔진 및 결과:                 │
│                                     │
│ 1. Local AI                         │
│    상태: ❌ 실패                     │
│    원인: Mixed Content Policy       │
│    메시지: HTTPS에서 HTTP 차단       │
│                                     │
│ 2. WebLLM                           │
│    상태: ❌ 타임아웃                 │
│    원인: 모델 다운로드 지연           │
│    메시지: 30초 타임아웃 초과        │
│                                     │
│ 3. Cloud API                        │
│    상태: ⏭️ 건너뜀                  │
│    원인: API 키 미설정               │
│                                     │
│ ┌───────────────────────────────┐   │
│ │ 관리자 설정에서 Cloud API를    │   │
│ │ 활성화하면 Fallback 품질이     │   │
│ │ 향상됩니다.                    │   │
│ └───────────────────────────────┘   │
│                                     │
│ [Fallback으로 진행] [설정으로 이동]  │
└─────────────────────────────────────┘
```

---

## 7. Fallback Content 개선

### 7.1 현재 Fallback 문제

```javascript
// 현재 (단순)
function createFallbackContent(text, title) {
  return {
    title,
    sections: [{ title: '내용', content: text }],
    quiz: createPlaceholderQuiz(10, title),  // "[자동 생성]" 표시
  };
}
```

### 7.2 개선된 Fallback Content

```javascript
/**
 * 개선된 Fallback Content 생성
 * - 원본 텍스트를 단락별로 분리
 * - 키워드 기반 자동 퀴즈 생성
 * - Fallback 사유 및 재생성 옵션 포함
 */
function createFallbackContent(text, title, errors = []) {
  // 1. 텍스트 단락 분리
  const paragraphs = text.split(/\n\n+/).filter(p => p.trim().length > 50);

  // 2. 섹션 자동 구성
  const sections = paragraphs.length > 1
    ? paragraphs.slice(0, 5).map((p, i) => ({
        title: `섹션 ${i + 1}`,
        content: `<p>${p.replace(/\n/g, '</p><p>')}</p>`,
      }))
    : [{ title: '학습 내용', content: `<p>${text}</p>` }];

  // 3. 키워드 추출 기반 퀴즈 생성
  const keywords = extractKeywords(text);  // 명사, 전문용어 추출
  const quiz = generateKeywordQuiz(keywords, title, 10);

  // 4. 메타 정보 포함
  return {
    title,
    sections,
    quiz,
    summary: `이 문서는 AI 분석 없이 원본 텍스트로 등록되었습니다. (${new Date().toLocaleString()})`,
    estimated_minutes: Math.max(5, Math.ceil(text.length / 500)),

    // Fallback 전용 메타데이터
    _fallback: {
      reason: errors.map(e => `${e.engine}: ${e.error}`).join('; '),
      timestamp: new Date().toISOString(),
      regeneratable: true,  // 나중에 AI로 재생성 가능
    },
  };
}

/**
 * 키워드 기반 퀴즈 생성 (더미가 아닌 실제 내용 기반)
 */
function generateKeywordQuiz(keywords, title, count) {
  const templates = [
    { q: '다음 중 ${title}에서 다루는 주요 개념은?', type: 'concept' },
    { q: '${keyword}의 의미로 올바른 것은?', type: 'definition' },
    { q: '${title} 관련 용어 중 틀린 것은?', type: 'wrong' },
  ];

  return keywords.slice(0, count).map((keyword, i) => {
    const template = templates[i % templates.length];
    return {
      question: template.q.replace('${keyword}', keyword).replace('${title}', title),
      options: generateOptions(keyword, keywords),
      answer: 0,
      explanation: `${keyword}는 이 문서의 핵심 개념입니다.`,
      isAuto: true,  // 자동 생성 표시
    };
  });
}
```

---

## 8. 구현 계획

### Phase 1: Fallback 안정화 (우선)
- [ ] WebLLM 타임아웃 추가 (30초/60초)
- [ ] Fallback Content 개선 (키워드 퀴즈)
- [ ] 에러 상태 UI 개선
- [ ] 테스트 코드 추가

### Phase 2: Docker 배포 옵션
- [x] docker-compose.yml 작성 (앱만 배포, 외부 AI 연동)
- [x] Dockerfile.dev 작성 (Node.js serve 기반)
- [x] 배포 가이드 문서화 (README.md)
- [ ] 실제 배포 테스트

### Phase 3: Cloud API 통합
- [ ] Gemini API 서비스 모듈
- [ ] 관리자 설정 UI
- [ ] 사용량 추적

### Phase 4: HTTPS Local AI (선택)
- [ ] Cloudflare Tunnel 가이드
- [ ] 환경 변수 분기 처리

---

## 9. 테스트 시나리오

| # | 시나리오 | 예상 결과 |
|---|----------|----------|
| 1 | localhost + Local AI | Local AI 성공 |
| 2 | Vercel + Local AI | Mixed Content 차단 → WebLLM 전환 |
| 3 | WebLLM 30초 초과 | 타임아웃 → Fallback |
| 4 | 모든 엔진 실패 | Fallback Content 생성 |
| 5 | Docker 배포 | Local AI 정상 동작 |

---

## 10. 성공 지표

- [ ] WebLLM 무한 로딩 제거 (30초 타임아웃)
- [ ] Fallback Content 100% 동작
- [ ] 에러 발생 시 명확한 사유 표시
- [ ] Docker 배포 1시간 내 완료 가능

---

## 참고

- PRD 0006: Gemini API Cloud Fallback
- Issue #103: AI Engine Selector UI
- Issue #101: Local AI Integration
